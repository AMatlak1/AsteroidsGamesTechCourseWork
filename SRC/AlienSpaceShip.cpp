...
void AlienSpaceShip::Shoot(void)
{
	...
	if (!mWorld) return;
	// Construct a unit length vector in the direction the alienspaceship is headed
	GLVector3f enemy_heading(cos(DEG2RAD * mAngle), sin(DEG2RAD * mAngle), 0);
	enemy_heading.normalize();
	// Calculate the point at the node of the alienspaceship from position and heading
	GLVector3f bullet_position = mPosition + (enemy_heading * 12);
